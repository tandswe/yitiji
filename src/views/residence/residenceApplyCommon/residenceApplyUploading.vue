    ...
    <div class="warp">   
        <div v-if="showEssent">
            ...
            <div class="legalCenter">
                <h1>申请人身份证明</h1>  
                <img :src="myPhoto" alt="" v-for="(col, index) of 1" :key="index">
                <button @click="editPromise(0)">修改</button>
            </div>
           <div class="warp-hr"></div> <!-- 换hr线的样式 -->
            <div class="legalCenterTwo">
                <h1>合法稳定住所证明</h1>
                <img :src="myPhoto" alt="" v-for="(col, index) of 1" :key="index">
                <button @click="editPromise(1)">修改</button>
            </div>
            <div class="warp-hr"></div> <!-- 换hr线的样式 -->
        </div>
 ...
 import buttonSub from "@/components/base/buttonSub"
    export default {
        ...
        methods:{
            ...
            editPromise(id) {
              if (id == 0) {
                this.$router.push('/residenceApplyQualification') // 跳转dao 03上传文件
                this.$store.commit('changeTabIndex',{count: 2})

              } else {
                this.$router.push({path: '/residenceApplyCondition', query: {promiseType: id}})
              }
            }
        }
    }
